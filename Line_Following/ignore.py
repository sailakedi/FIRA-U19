...
def check_green_markers(green_contours, black_mask, frame_width, frame_height):
    ...
    for contour in green_contours:
        ...
        # Get bounding box of green marker
        rect = cv2.minAreaRect(contour)
        box = cv2.boxPoints(rect)
        box = np.int0(box)
        
        # Sort box points by y coordinate
        box = box[box[:, 1].argsort()]
        marker_height = box[-1][1] - box[0][1]
        
        # Check if marker is at bottom of frame
        is_at_bottom = box[2][1] > frame_height * 0.95
        
        # Sort by x coordinate for left/right analysis
        box_x_sorted = box[box[:, 0].argsort()]
        
        # Define regions around the green marker to check for black
        # Top region
        ...
        # Check for black in each region
        black_top = False
        black_left = False
        black_right = False
        ...
        # Determine turn direction based on black pattern
...
def process_frame(frame, line_tracker):
    ...
    # Green marker detection and turning logic
