...
def check_green_markers(green_contours, black_line_y, frame_width):
    ...
    if left_markers and right_markers:
        ...
        return True, False
    elif left_markers_above and right_markers_above:
        return False, True
    elif left_markers_above or right_markers_above:
        return False, True
...
def process_frame(frame, line_tracker):
    ...
    red_mask = cv2.morphologyEx(red_mask, cv2.MORPH_OPEN, kernel)  # <<< ADD RED MORPH

    black_contours, _ = cv2.findContours(black_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    green_contours, _ = cv2.findContours(green_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    red_contours, _ = cv2.findContours(red_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # <<< RED CONTOURS

    # >>> RED LINE REACTION >>>
    if red_contours:
        largest_red = max(red_contours, key=cv2.contourArea)
        if cv2.contourArea(largest_red) > 300:
            print("Red line detected! Moving forward briefly.")
            line_tracker.send_command('F')
            time.sleep(0.5)
            line_tracker.send_command('S')
            return roi.copy(), black_mask, 'S', None, None, False, None, "Red Line Detected"
    # <<< RED LINE REACTION <<<
    ...
    for contour in green_contours:
        ...
        if area > 300:
            ...
            if center_point is not None and y > center_point[1]:
                cv2.putText(display_roi, 'Green Below', (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
            else:
                cv2.putText(display_roi, 'Green Above', (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
    ...
    if not line_detected:
        cv2.putText(display_roi, "Line Lost!", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
